...
C      count = 0
c      do
c      read(IUNIT, iostat=ios) IWI4,JWI4,XFINT,YFINT,DXINT,
c     +                  DYINT, NFREC4, WDAY
C     if (ios /= 0) exit ! exit the loop if an error occurred or end of file reached
C     !count = count + 1
C     !end do
C     !print *, 'Number of records in the file:', count
     ...
     +            NFREC4,WDAY(1:6000)
      ...
      IF     (NREC.GE.9000) THEN
        WRITE(6,9150) NREC
        print *, CTITLE,IWI,JWI,NREC
        CALL ZHFLSH(6)
C        NREC = 6000 
C      XING MODIFY THE CODE TO LIMIT THE VALUE
        STOP
...
C
 9150 FORMAT(// 20X,'*****  ERROR IN FREAD0  -  ',
     +   'NFREC LARGER THAN 8999 (NFREC = ',I11,')  *****' //)
