...
%-------------------------------------------------------
%%
pv_t_filename = strcat('seamount_2019_2020_pv_t_vslice_const_lon_nt_',string(indxRange(1)), '_', ...
...
lon_idx_arr = ncread(pv_t_filename, 'lon_idx');
lon_arr = ncread(pv_t_filename, 'lon');
...
%%
%-------------------------------------------------------
rho_pot_t_filename = strcat('seamount_2019_2020_rho_pot_t_vslice_const_lon_nt_',string(indxRange(1)), '_', ...
    string(indxRange(Nt)),'.nc');
% rho_pot_t_filename = strcat('seamount_2019_2020_rho_pot_t_vslice_const_lat_nt_',string(rho_indxRange(1)), '_', ...
%     string(rho_indxRange(rho_Nt)),'.nc');
...
for nt = 1:Nt
    ...
    for j = 1:length(lon_arr)
        ...
        ax1 = axes('Parent', figure1, 'YMinorTick','on',...
            ...
            'FontSize',24);
        hold on;
        epv_vslice = squeeze(pv_t_vslice(nt, :, :, j));
        ...
        % colormap(ax1, "jet"); 
        colormap(cmocean('balance')); cb = colorbar;
        cb.FontSize = 18;
        ...
        if (strcmp(sec, 'zoom')==true)
            figname  = [plots_path, 'si_check/vslice/const_lon_zoom/seamount_2019_2020_pv_t_vslice'];
        elseif(strcmp(sec, 'nozoom')==true)
            figname  = [plots_path, 'si_check/vslice/const_lon/seamount_2019_2020_pv_t_vslice'];
        end
        % 
