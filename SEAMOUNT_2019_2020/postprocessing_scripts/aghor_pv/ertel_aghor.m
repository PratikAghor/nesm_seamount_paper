function pv = ertel_aghor(fname,gname,lambda,tindex);                     
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
% aghoredit, read pm, pn from gridfile, not fname 
gc = netcdf(gname);
pm = tridim(gc{'pm'}(:), N);
pn = tridim(gc{'pn'}(:), N);
f=tridim(gc{'f'}(:),N);
...
% aghoredit made v(:, 1:end, 2:end) -> v(:, 1:end-1, 2:end) for compatible sizes for .*
