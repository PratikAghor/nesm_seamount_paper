...
indxRange = 3016:3072; % Jan 14 - Jan 20, 2020nt0=indxRange(1);
[~, Nt] = size(indxRange);
%-------------------------------------------------------
%-------------------------------------------------------
%%
...
vebf_vslice_arr = (ncread(filename, 'vebf_vslice'));
...
for i = 1:length(lat_idx_arr)
    ...
    hold on;
    hrs_vslice = squeeze(vebf_vslice_arr(:, i, :));
    zMin = -5e-7; 
    ...
    figname  = [plots_path, 'rs_plots/vebf/seamount_2019_2020_vebf_vslice'];
