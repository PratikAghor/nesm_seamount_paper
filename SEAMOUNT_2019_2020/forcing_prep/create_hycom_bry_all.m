...
nc = netcdf([in_begin,'water_temp',in_end])
lonT = nc{'lon'}(:);
latT = nc{'lat'}(:);
depth = -nc{'depth'}(:);
time = nc{'time'}(:)/24 + datenum('2000-01-01 00:00:00') - datenum(Yorig,1,1);
temp = nc{'water_temp'}(:);
max(temp(:))
min(temp(:))
close(nc)
nc = netcdf([in_begin,'salinity',in_end])
time_tmp = nc{'time'}(:)/24 + datenum('2000-01-01 00:00:00') - datenum(Yorig,1,1);
...
min(salt(:))
close(nc)
nc = netcdf([in_begin,'water_u',in_end])
time_tmp = nc{'time'}(:)/24 + datenum('2000-01-01 00:00:00') - datenum(Yorig,1,1);
if ~isequal(time_tmp,time)
...
max(u(:))
min(u(:))
nc = netcdf([in_begin,'water_v',in_end])
time_tmp = nc{'time'}(:)/24 + datenum('2000-01-01 00:00:00') - datenum(Yorig,1,1);
if ~isequal(time_tmp,time)
...
max(v(:))
min(v(:))
nc = netcdf([in_begin,'surf_el',in_end])
time_tmp = nc{'time'}(:)/24 + datenum('2000-01-01 00:00:00') - datenum(Yorig,1,1);
if ~isequal(time_tmp,time)
