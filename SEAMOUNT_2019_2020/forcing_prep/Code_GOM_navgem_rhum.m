...
varname = 'rhum';
longname_var = 'relative humidity';
unit_var = 'fraction';

disp(['Reading data ...'])

time1 = ncread(spchumfile1,'MT');
II = find(time1>=t_min & time1<=t_max);
II = II(1:dt:end);
time11 = time1(II);
Nt1 = length(II)
tt1 = ncread(spchumfile1,var_in,[1 1 II(1)],[Inf Inf length(II)],[1 1 dt]);

if time11(end)<t_max
  time2 = ncread(spchumfile2,'MT');
  II = find(time2>=(time11(end)+dt/24) & time2<=t_max);
  II = II(1:dt:end);
  time2 = time2(II);
  Nt2 = length(II);
  tt2 = ncread(spchumfile2,var_in,[1 1 II(1)],[Inf Inf length(II)],[1 1 dt]);
else
  Nt2 = 0;
end
time1 = time11;
...
lon0 = ncread(spchumfile1,'Longitude')-360;
lat0 = ncread(spchumfile1,'Latitude');
