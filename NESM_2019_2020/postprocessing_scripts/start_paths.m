...
% main_dir_path='/home/paghor3/';
% aghor_extras_path = '/media/paghor3/aghor/prometheus_backup/Aghor/GT/GT_project/aghor_extras/';
%------------------------------------
plots_path='../plots/';
...
mask_psi=pagetranspose(ncread(gridfile, 'mask_psi'));
...
hc = ncread(gridfile, 'hc');lon_rho_vec = squeeze(lon_rho(1, :));
...
[Ny, Nx] = size(lat_rho);
latlim = [lat_rho_vec(1), lat_rho_vec(Ny)];
lonlim = [lon_rho_vec(1), lon_rho_vec(Nx)];
%--------------------------------------------------------------------------
...
time    = 0;
fname   = sprintf('NESM_2019_2020_avg.%05d.nc',time); hisfile = fname;
