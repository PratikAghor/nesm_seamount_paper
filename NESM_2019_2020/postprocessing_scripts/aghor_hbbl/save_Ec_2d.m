...
for nt = 1:Nt
    ...
    % ws is not the vertical velocity as it is
    % ws is the vertical velocity associated with the barotropic flow

    ub = pagetranspose(ncread(hisfile, 'ubar'));
    vb = pagetranspose(ncread(hisfile, 'vbar'));
    ...
    [Wb_at_rho_pts,Wb_at_w_pts]=get_wvelocity(zeta,ub,vb,depth,pm,pn,theta_s,theta_b,hc,N,Vtransform);
    ...
    % sigma4_  =  gsw_sigma4(salt, temp); % get potential density
   
    % rhotmp = rho0 + sigma4_;

    % if  ws is the vertical velocity associated with the barotropic flow
    ws = Wb_at_rho_pts;
       
    rhoprime = (rhotmp - rho0);

    rpws = g.*(rhoprime.*ws);
    
    [Ec, h0] = vintegr2((rpws), zw, zr, NaN, NaN); % perform vertical integration from bottom to top 
    Ec_avg = Ec_avg + (Ec);
end
...
filename = strcat('nesm_2019_2020_Ec_2d_nt_',string(indxRange(1)), '_', ...
