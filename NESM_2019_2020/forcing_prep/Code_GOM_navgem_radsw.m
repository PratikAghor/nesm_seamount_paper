...
varname = 'radsw';
longname_var = 'solar shortwave radiation';
unit_var = 'Watts meter-2';
pos_var = 'downward flux, warming water';

disp(['Reading data ...'])

time1 = ncread(swradfile1,'MT');
II = find(time1>=t_min & time1<=t_max);
II = II(1:dt:end);
time11 = time1(II);
Nt1 = length(II);
tt1 = ncread(swradfile1,var_in,[1 1 II(1)],[Inf Inf length(II)],[1 1 dt]);

if time11(end)<t_max
  time2 = ncread(swradfile2,'MT');
  II = find(time2>=(time11(end)+dt/24) & time2<=t_max);
  II = II(1:dt:end);
  time2 = time2(II);
  Nt2 = length(II);
  tt2 = ncread(swradfile2,var_in,[1 1 II(1)],[Inf Inf length(II)],[1 1 dt]);
else
  Nt2 = 0;
end
time1 = time11;
...
lon0 = ncread(swradfile1,'Longitude')-360;
lat0 = ncread(swradfile1,'Latitude');
